...
|---|---|---|
| `config/model.yaml` | `application/scoring.py::build_scores`; `model_version` gravado no snapshot por `application/history.py::save_history_snapshot` | `model_version: "0.3"`; pesos business 0.35 / valuation 0.30 / financial 0.15 / timing 0.20; confiança limitada a 59 quando falta feature `required` |
...
- **`watchlist/promote.py`**: gate manual **por design**, não lacuna — confirmado com o usuário (2026-07-18). `run_all.py` nunca escreve em `config/watchlist.csv`; a decisão de promover um símbolo continua sempre do usuário (mesma decisão "propor-só" do PR-021). O que mudou: `watchlist/candidates_workbook.py` + `watchlist/apply_candidates_workbook.py` dão uma interface Excel (marcar "Incluir" numa lista completa de candidatos, ou digitar qualquer ticker à mão) em cima do mesmo `promote_to_watchlist` — mais amigável, mesmo gate. Ver `docs/WATCHLIST_WORKBOOK.md`.
