...
def build_company_reports(df: pd.DataFrame)->list[CompanyReport]:
    ...
    for _,row in df.iterrows():
        reports.append(CompanyReport(
            ...
            data_coverage=row.get("Data Coverage"),
            source_quality=row.get("Source Quality"),
            data_freshness=row.get("Data Freshness"),
            risk_penalty=row.get("Risk Penalty"),
            observed_risk_penalty=row.get("Observed Risk Penalty"),
            risk_uncertainty_penalty=row.get("Risk Uncertainty Penalty"),
            missing_required_features=row.get("Missing Required Features"),
            risk_evidence_missing=row.get("Risk Evidence Missing"),
            reference_universe=row.get("reference_universe", ""),
