...
@dataclass
class SecSharesCache:
    """Per-symbol TTL cache for SEC-derived shares_outstanding records.

    Longer default TTL than price/market-cap caches (see builder default):
    shares_outstanding only changes when a new 10-Q/10-K is filed
    (quarterly), so re-fetching every run wastes SEC EDGAR calls for data
    that has not moved.
    """
