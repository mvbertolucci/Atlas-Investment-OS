...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Preenche o snapshot paginado Massive Float."
    )
    ...
    max_pages = (
        ...
        else int(settings.get("massive_float_max_pages_per_run", 100))
    )
    summary = provider.prefetch_float_universe(
        symbols, max_pages=max_pages
    )
    source_payload = (
    ...
    report_path = Path(
        str(
            settings.get(
                "massive_float_coverage_report_path",
                "output/dados/massive_float_coverage.json",
            )
        )
    )
