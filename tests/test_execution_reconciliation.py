...
def test_classifies_confirmed_partial_not_reflected_variance_and_unverifiable() -> None:
    report = reconcile_executions(
        ...
        baseline_snapshot_at="2026-07-21T10:00:00", current_snapshot_at="2026-07-23T10:00:00",
        generated_at="now",
    ).to_dict()
...
def test_ignores_execution_at_or_before_baseline() -> None:
    ledger = _ledger(("AAA", 5), ("BBB", 2))
    ledger["events"][0]["executed_at"] = "2026-07-20T10:00:00"
    report = reconcile_executions(
        ledger, baseline_portfolio=_portfolio(AAA=10, BBB=10),
        current_portfolio=_portfolio(AAA=10, BBB=8),
        baseline_snapshot_at="2026-07-21T10:00:00",
        current_snapshot_at="2026-07-23T10:00:00",
    )
...
def test_cli_writes_report(tmp_path: Path, capsys) -> None:
    ...
    main([str(baseline), str(current), "--baseline-at", "2026-07-21T10:00:00", "--current-at",
