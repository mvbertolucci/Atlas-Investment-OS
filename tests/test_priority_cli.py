...
research_ranking_report.json, portfolio.csv) -- the part the pure pipeline
tests in test_priority_pipeline.py cannot cover.
"""
...
def test_build_priority_reads_sell_side_from_disk(tmp_path: Path) -> None:
    ...
    assert [item.symbol for item in report.sell.items] == ["AAA", "BBB"]
...
def test_build_priority_handles_missing_portfolio_file(tmp_path: Path) -> None:
    ...
    # No held-symbol filter is applied when the portfolio file is absent.
    assert [item.symbol for item in report.sell.items] == ["AAA"]
