...
def test_empty_build_is_valid_and_versioned() -> None:
    ...
    assert data["decision_journal"] is None
    assert data["execution_ledger"] is None
    assert data["execution_reconciliation"] is None
...
def test_all_views_assembled_from_domain_objects() -> None:
    ...
    assert data["execution_ledger"]["fills"] == 2
